...
class mlp:
    ...
    def add_layer(self, number_of_nodes: int,function="sigmoid"):
        ...
        if len(self.number_of_nodes) > 1:
            self.weights.append(np.random.randn(self.number_of_nodes[-1], self.number_of_nodes[-2])*2.5*((np.square(self.number_of_nodes[-1])+np.square(self.number_of_nodes[-2]))/np.square(self.number_of_nodes[-1]+self.number_of_nodes[-2])))
        ...
        if len(self.number_of_nodes) > 1:
            ...
            self.biases.append(np.random.uniform(0, 0.01, size=(number_of_nodes, 1)))
